...
export async function GET(req: Request) {
  try {
    ...
    const items = rawItems.map(
      ({ currency, updatedAt, createdAt, __v, kibooId, ...p }) => {
        ...
        const salePrice =
          p.salePrice && p.salePrice > 0
            ? isUSD
              ? Math.round((p.salePrice * dolarPrice + Number.EPSILON) * 100) /
                100
              : p.salePrice
            : null;
        return {
    ...
    const productsPromotion = rawproductsPromotion.map(
      ({ currency, updatedAt, createdAt, __v, kibooId, ...p }) => {
        const isUSD = currency == "USD";
        const price = isUSD
          ? Math.round((p.price * dolarPrice + Number.EPSILON) * 100) / 100
...
export async function PUT(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: "No se encontró el ID del producto" },
        { status: 400 },
      );
    }
